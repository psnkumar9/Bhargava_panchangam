...
def get_dhasa_bhukthi(
    ...
    """
        ...
        @param round_duration: If True, round only the returned duration values to
                               dhasa_level_index; internal calcs use full precision.
...
def dwadasottari_immediate_children(
    ...
    *,
    jd_at_dob,
    place,
    # REQUIRED knobs for this dasha:
    antardhasa_option: int = 1,
    # Optional extras supported by your base (accepted, but not needed here):
    **kwargs
):
    """
    Dwādaśottarī — return ONLY the immediate (p -> p+1) children under the given parent span.

    Rules (matches your base get_dhasa_bhukthi()):
      • Child order at every level from your antara logic: _antardhasa(parent_lord, antardhasa_option)
      • Equal split at this level: child_years = parent_years / len(children)
    ...
    def _children_of(pl):
        # uses your module's internal helper (same as in base get_dhasa_bhukthi)
...
def get_running_dhasa_for_given_date(
    ...
    *,
    ...
    antardhasa_option: int = 1,
    # Forwarded options (same names as your base Dwādaśottarī get_dhasa_bhukthi):
    star_position_from_moon=1,
    use_tribhagi_variation=False,
    divisional_chart_factor=1,
    chart_method=1,
    seed_star=27,
    dhasa_starting_planet=1,
    round_duration=False,        # runner operates on starts/ends; durations rounding not needed
    **kwargs                     # tolerated extras
):
    """
    Dwādaśottarī — narrow Mahā -> … -> target level and return the full running ladder:
    ...
    target_depth = max(const.MAHA_DHASA_DEPTH.MAHA_DHASA_ONLY,
                       min(const.MAHA_DHASA_DEPTH.DEHA, target_depth))

    # ---- derive dob/tob (if needed somewhere later)
    # (Dwādaśottarī base uses dob/tob; your immediate children do not need them.)
    y, m, d, fh = utils.jd_to_gregorian(jd_at_dob)
    dob = drik.Date(y, m, d)
    tob = (fh, 0, 0)

    # ---- Level 1: Mahā via your Dwādaśottarī base
    maha_rows = get_dhasa_bhukthi(
        ...
        round_duration=False,  # runner doesn't need rounded durations
    )
