...
def get_dhasa_bhukthi(
    ...
    """
        ...
        @param dhasa_level_index: Depth (1..6)
            ...
            5 = + Prana
            6 = + Deha-antara
        @param round_duration: If True, round only the returned duration to dhasa_level_index
...
def nakshathra_dhasa_progression(
    ...
    vd = get_dhasa_bhukthi(dob, tob, place, star_position_from_moon, use_tribhagi_variation,
                             divisional_chart_factor, chart_method, seed_star, dhasa_starting_planet,
                             antardhasa_option, dhasa_level_index=DLI)
...
def dwisatpathi_immediate_children(
    ...
    *,
    ...
    # Optional extras (accepted, ignored here):
    **kwargs
):
    """
    Dwisatpathi — return ONLY the immediate (p -> p+1) children under the given parent span.

    Rules (matches your base get_dhasa_bhukthi()):
      • Child order from your antara logic: _antardhasa(parent_lord, antardhasa_option)
    ...
    def _children_of(pl):
        # Same helper your base function uses:
    ...
    for i, cl in enumerate(child_lords):
        if i == n - 1:
            child_end = end_jd                # force closure
        else:
...
def get_running_dhasa_for_given_date(
    ...
    *,
    # REQUIRED knob for this dasha:
    antardhasa_option: int = 1,
    # Forwarded options (same names as your base dwisatpathi get_dhasa_bhukthi):
    star_position_from_moon=1,
    use_tribhagi_variation=False,
    divisional_chart_factor=1,
    chart_method=1,
    ...
    **kwargs
    ...
    """
    Dwisatpathi — narrow Mahā -> … -> target level and return the full running ladder:
    ...
    # ---- derive dob/tob for the base generator
    y, m, d, fh = utils.jd_to_gregorian(jd_at_dob)
    dob = drik.Date(y, m, d)
    tob = (fh, 0, 0)
    ...
    for depth in range(2, target_depth + 1):
        ...
        # Expand only this parent (equal split + antara order)
